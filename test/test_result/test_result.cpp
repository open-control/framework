...
void test_result_void_ok() {
    auto result = oc::type::Result<void>::ok();
...
void test_result_void_err() {
    auto result = oc::type::Result<void>::err({oc::type::ErrorCode::HARDWARE_INIT_FAILED, "test"});
    TEST_ASSERT_FALSE(result.isOk());
    TEST_ASSERT_TRUE(result.isErr());
    TEST_ASSERT_FALSE(static_cast<bool>(result));
    TEST_ASSERT_EQUAL(oc::type::ErrorCode::HARDWARE_INIT_FAILED, result.error().code);
    TEST_ASSERT_EQUAL_STRING("test", result.error().context);
}

void test_result_void_err_without_context() {
    auto result = oc::type::Result<void>::err({oc::type::ErrorCode::INVALID_STATE});
    TEST_ASSERT_TRUE(result.isErr());
    TEST_ASSERT_EQUAL(oc::type::ErrorCode::INVALID_STATE, result.error().code);
    TEST_ASSERT_NULL(result.error().context);
}

void test_result_value_ok() {
    auto result = oc::type::Result<int>::ok(42);
    TEST_ASSERT_TRUE(result.isOk());
    TEST_ASSERT_EQUAL(42, result.value());
}

void test_result_value_err() {
    auto result = oc::type::Result<int>::err({oc::type::ErrorCode::INVALID_ARGUMENT, "param"});
    TEST_ASSERT_TRUE(result.isErr());
    TEST_ASSERT_EQUAL(oc::type::ErrorCode::INVALID_ARGUMENT, result.error().code);
    TEST_ASSERT_EQUAL_STRING("param", result.error().context);
}

void test_result_value_or() {
    auto ok = oc::type::Result<int>::ok(42);
    auto err = oc::type::Result<int>::err({oc::type::ErrorCode::RESOURCE_NOT_FOUND});
...
void test_result_map() {
    auto result = oc::type::Result<int>::ok(21);
...
void test_result_map_err_propagates() {
    auto result = oc::type::Result<int>::err({oc::type::ErrorCode::INVALID_STATE});
    auto mapped = result.map([](int x) { return x * 2; });
    TEST_ASSERT_TRUE(mapped.isErr());
    TEST_ASSERT_EQUAL(oc::type::ErrorCode::INVALID_STATE, mapped.error().code);
}

void test_result_or_else() {
    auto err = oc::type::Result<int>::err({oc::type::ErrorCode::RESOURCE_NOT_FOUND});
    auto recovered = err.orElse([](oc::type::Error) { return oc::type::Result<int>::ok(99); });
...
void test_result_or_else_on_ok() {
    auto ok = oc::type::Result<int>::ok(42);
    bool called = false;
    auto result = ok.orElse([&called](oc::type::Error) {
        called = true;
        return oc::type::Result<int>::ok(99);
    });
...
void test_error_code_to_string() {
    TEST_ASSERT_EQUAL_STRING("HARDWARE_INIT_FAILED",
                             errorCodeToString(oc::type::ErrorCode::HARDWARE_INIT_FAILED));
    TEST_ASSERT_EQUAL_STRING("OK", errorCodeToString(oc::type::ErrorCode::OK));
    TEST_ASSERT_EQUAL_STRING("STORAGE_CORRUPT", errorCodeToString(oc::type::ErrorCode::STORAGE_CORRUPT));
    TEST_ASSERT_EQUAL_STRING("CONTEXT_INIT_FAILED",
                             errorCodeToString(oc::type::ErrorCode::CONTEXT_INIT_FAILED));
}

void test_error_code_unknown() {
    // Cast invalid value to test default case
    auto unknown = static_cast<oc::type::ErrorCode>(255);
    TEST_ASSERT_EQUAL_STRING("UNKNOWN", errorCodeToString(unknown));
}

void test_result_move_semantics() {
    auto result1 = oc::type::Result<int>::ok(42);
...
void test_result_move_assignment() {
    auto result1 = oc::type::Result<int>::ok(42);
    auto result2 = oc::type::Result<int>::err({oc::type::ErrorCode::INVALID_STATE});
